...
export default function CommunityGroups() {
  ...
  useEffect(() => {
    const checkAuthAndFetchData = async () => {
      ...
      setIsTeacher(roleData && roleData.length > 0);
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: { user } } = await supabase.auth.getUser();
      const { error } = await supabase.from("conversation_groups").insert({
      ...
      setGroupForm({ name: "", description: "", is_visible: true, students_can_message: true });
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="space-y-1">
          {groups.map(group => {
            ...
            const timeString = lastMessageTime ? lastMessageTime.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : '';
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground truncate">
                      {group.last_message ? 
                        `${group.last_message_sender || 'Alguém'}: ${group.last_message}` : 
                        group.description || "Sem mensagens ainda"
                      }
                    </p>
